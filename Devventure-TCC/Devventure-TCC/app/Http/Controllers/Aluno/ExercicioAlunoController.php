...
public function responder(Request $request, Exercicio $exercicio)
{
    ...
    // Verificações de segurança (continuam iguais)
    if (! $aluno->turmas->contains($exercicio->turma_id)) {
        abort(403, 'Acesso não autorizado a este exercício.');
    }
    ...
    foreach ($request->file('arquivos_resposta') as $arquivo) {
        ...
        if (!in_array($extension, $allowedExtensions)) {
            ...
            return redirect()->back()->withErrors([
                'arquivos_resposta' => 'O formato de arquivo "' . $extension . '" não é permitido. Permitidos: pdf, docx, zip, png, jpg, java, txt.'
            ])->withInput(); 
        }
    }
    ...
    //  Se todas as extensões estiverem OK, o código de salvamento continua
    try {
        DB::transaction(function () use ($request, $exercicio, $aluno) {
            ...
            // Lógica para salvar os novos arquivos
            foreach ($request->file('arquivos_resposta') as $arquivo) {
